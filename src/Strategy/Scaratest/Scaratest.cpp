...
void Scaratest::cassette(int id, int drawer, bool io) {
    ...
    go_straight(ready, CASAY[id], (CASAY[id][5] + drawer * DRAWER_H + (int)io * LIFT_DIS), DIV);    // Move from ready to cassette
    io = !io;   // inverse io value for easier caculation

    // printf("io = %d\n", io);    // FOR DEBUG
    go_straight(CASAY[id], CASAY[id], (CASAY[id][5] + drawer * DRAWER_H + (int)io * LIFT_DIS), 1);  // Lift/place waffer
    go_straight(CASAY[id], ready, (CASAY[id][5] + drawer * DRAWER_H + (int)io * LIFT_DIS), DIV);    // Move back to ready position
}
...
void Scaratest::station(int id, bool io) {
    ...
    go_straight(ready, STATE[id], (STATE[id][5] + (int)io * (LIFT_DIS + STAT_SHIFT)), DIV);    // Move from ready to cassette

    // io = !io;    // FOR DEBUG
    printf("io = %d\n", io);
    go_straight(STATE[id], STATE[id], (STATE[id][5] + (int)io * (LIFT_DIS + STAT_SHIFT)), 1);  // Lift/place waffer
    go_straight(STATE[id], ready, (STATE[id][5] + (int)io * (LIFT_DIS + STAT_SHIFT)), DIV);    // Move back to ready position
}
