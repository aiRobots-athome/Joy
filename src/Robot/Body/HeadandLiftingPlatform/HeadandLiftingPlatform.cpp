#include "HeadandLiftingPlatform.h"

HeadandLiftingPlatform::HeadandLiftingPlatform(const vector<unsigned char> &IDArray,
...
HeadandLiftingPlatform::~HeadandLiftingPlatform(void)
{
}
void HeadandLiftingPlatform::InitHeadMotorVelocity(void)
{
	this->SetMotor_Velocity(FIRST_HEAD_MOTOR_ID, 200);			// Mx64
	this->SetMotor_Velocity(FIRST_HEAD_MOTOR_ID + 1, 200);		// Mx64
}
void HeadandLiftingPlatform::HeadMotorCommand(const int &MotorID, const float &Angle)
{
	// Bcuz SetMotor_Angle is protected in MotorUnion, it must be implemented in HeadandLiftingPlatform
	this->SetMotor_Angle(MotorID, Angle);
}
void HeadandLiftingPlatform::ResetAllMotorAngle()
{
	this->SetMotor_Angle(FIRST_HEAD_MOTOR_ID, 0);
	this->SetMotor_Angle(FIRST_HEAD_MOTOR_ID + 1, 0);
	this->WaitAllMotorsArrival();
}
void HeadandLiftingPlatform::HeadCoordinate(float X_kinect, float Y_kinect, float depth, float &X_real, float &Y_real, float &Z_real)
{
	int type = 1;
...
void HeadandLiftingPlatform::RobotToCamera(float X_real, float Y_real, float Z_real, float &X_kinect, float &Y_kinect, float &depth)
{
	float kx, ky, kz;
...
void HeadandLiftingPlatform::Get_T_CamCoord2RobotCoord()
{
...
void HeadandLiftingPlatform::HeadCoordinateKJ(float X_kinect, float Y_kinect, float depth, float &X_real, float &Y_real, float &Z_real)
{
...
void HeadandLiftingPlatform::GetObjectRotate_JU()
{
	float J1 = this->GetMotor_PresentAngle(FIRST_HEAD_MOTOR_ID);
