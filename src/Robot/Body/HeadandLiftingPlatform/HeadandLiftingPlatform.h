...
class HeadandLiftingPlatform: public MotorUnion
{
    public:
        HeadandLiftingPlatform(const vector<unsigned char> &IDArray,
                               const vector<string> &MotorModelArray,
                               vector<unsigned char> &AllPortNumber);
        ~HeadandLiftingPlatform(void);

        /* Initialize */
        // Set in proper speed range for head
        void InitHeadMotorVelocity(void);

        /* Main Move Function */
        void HeadMotorCommand(const int &MotorID, const float &Angle);
        void ResetAllMotorAngle(void);
